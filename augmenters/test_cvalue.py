...
class TestChileanMorphosyntax(unittest.TestCase):
    ...
    def test_find_voceo_verbs(self):
        text = "¿Por qué no venís mañana? Siempre hablái así."
        results = self.parser.find_voceo_verbs(text)
        self.assertTrue(any("venís" in r["text"] for r in results))
        self.assertTrue(any("hablái" in r["text"] for r in results))
    ...
    def test_find_chilean_imperative(self):
        text = "Anda luego y pásame la llave."
        results = self.parser.find_chilean_imperative(text)
        self.assertTrue(any("Anda" in r["text"] for r in results))
        self.assertTrue(any("pásame" in r["text"] for r in results))
    ...
    def test_run_rules(self):
        text = "Cachai que me voy a irme y pásame la botella."
        results = self.parser.run_rules(text)
        # Should detect at least one of each type
        self.assertTrue(any("Cachai" in r["text"] for r in results))
        self.assertTrue(any("irme" in r["text"] for r in results))
        self.assertTrue(any("pásame" in r["text"] for r in results))
        self.assertTrue(any("que" in r["text"] for r in results))
